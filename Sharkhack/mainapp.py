...
def dashboard_profile():
    ...
    if not user.get("health_conditions_raw"):
        health_conditions = "N/A"
    else:
        health_conditions = user.get("health_conditions_raw")

    if not user.get("dietary_restrictions_raw"):
        dietary_restrictions = "N/A"
    else:
        dietary_restrictions = user.get("dietary_restrictions_raw")

    if not user.get("allergies_raw"):
        allergies = "N/A"
    else:
        allergies = user.get("allergies_raw")
    return render_template("dashboard_profile.html",
                           ...
                           h_health_conditions=health_conditions,
                           h_dietary_restrictions=dietary_restrictions,
                           h_allergies=allergies,
                           user=user)
...
def profile_update():
    ...
    if request.method == 'POST':
        ...
        if not user.get('health_conditions_raw'):
            user['health_conditions_raw'] = health_raw
        if not user.get('dietary_restrictions_raw'):
            user['dietary_restrictions_raw'] = diet_raw
        if not user.get('allergies_raw'):
            user['allergies_raw'] = allergy_raw
        updateData['health_conditions_raw'] = health_raw
        ...
        if not user.get('health_conditions_normalized'):
            user['health_conditions_normalized'] = health_normalized
        if not user.get('dietary_restrictions_normalized'):
            user['dietary_restrictions_normalized'] = diet_normalized
        if not user.get('allergies_normalized'):
            user['allergies_normalized'] = allergy_normalized
        # Update normalized fields in updateData    
        updateData['health_conditions_normalized'] = health_normalized
